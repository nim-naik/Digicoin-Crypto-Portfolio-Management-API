...
# --- FastAPI app ---
app = FastAPI(title="Digicoin API", docs_url="/docs", redoc_url="/redoc")
...
def register(user: UserCreate, db: Session = Depends(get_db)):
    ...
    return {'message': f'Successfully created new {user.username}.'}
...
def buy_asset(trade: TradeAsset, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    transaction = Transaction(portfolio_id=portfolio.id, symbol=trade.symbol, quantity=trade.quantity,
                              price=price, timestamp=datetime.utcnow())
    db.add(transaction)
    portfolio.available_money -= total_cost
    db.commit()
    return {'message': f'{asset.symbol} successfully bought.'}
...
def sell_asset(trade: TradeAsset, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.commit()
    return {'message': f'{asset.symbol} successfully sold.'}
...
def get_portfolio(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for asset in portfolio.assets:
        ...
        transactions = db.query(Transaction).filter(Transaction.portfolio_id == portfolio.id,
                                                    Transaction.symbol == asset.symbol).all()
        total_cost = sum(t.price * t.quantity for t in transactions if t.quantity > 0)
        total_bought = sum(t.quantity for t in transactions if t.quantity > 0)
        avg_purchase_price = total_cost / total_bought if total_bought else 0
    ...
    return {
        ...
        'performance_rel': (total_value - portfolio.total_added_money) / portfolio.total_added_money * 100
        if portfolio.total_added_money != 0 else 0,

...
def buy_asset(trade: TradeAsset, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if price is None or price <= 0:
        raise HTTPException(
            status_code=404,
            detail=f'Symbol "{trade.symbol}" not found or has no market value'
        )
...
def get_portfolio(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for asset in portfolio.assets:
        ...
        assets_response.append({
            ...
            'performance_rel': (asset_value - invested_amount) / invested_amount * 100 if invested_amount else 0
        })
    ...
    return {
        ...
        'performance_rel': (
                                   total_value - portfolio.total_added_money) / portfolio.total_added_money * 100 if portfolio.total_added_money else 0,
